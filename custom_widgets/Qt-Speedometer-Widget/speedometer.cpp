...
//    QString s_SpeedInt = QString::number(speedInt).append('.');
    QString s_SpeedInt = QString::number(speedInt); // ponto removido
    ...
    // Draw unit
    const float unitFactor = 0.4f;

    painter.setPen(m_unitTextColor);
    QString fontFamily = this->font().family();
    int baseSize = this->font().pointSize();
    QFont unitFont(fontFamily, int(baseSize * unitFactor));
    painter.setFont(unitFont);
    painter.drawText(unitRect, Qt::AlignCenter, m_unit);

    // Draw Speed
    const float speedIntFactor = 1.2f;
    const float speedDecFactor = 0.6f;

    QColor speedColor(0,0,0);
    QFont speedFont(fontFamily, int(baseSize * speedIntFactor));
    QFontMetrics fm1(speedFont);
    int speedWidth = fm1.width(s_SpeedInt);

//    QFont speedDecFont(fontFamily, int(baseSize * speedDecFactor));
//    QFontMetrics fm2(speedDecFont);
//    int speedDecWidth = fm2.width(s_SpeedDec);

//    int leftPos = -1 * (speedWidth + speedDecWidth) / 2.0 ;
    int leftPos = -1 * (speedWidth) / 2.0 ;
